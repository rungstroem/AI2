...
class myQPlayer : public iplayer{
	...
	private:
		...
		int make_decision(){
			...
			int state = state_translate(pos, piece);
			int action = qtable->getMaxQAction(state);
			
			int newPos = get_new_pos(piece);
			int newState = state_translate(newPos, piece);
			srand(time(NULL));
			...
			double Q = qtable->getQVal(state,action);
			double Qn = qtable->getQVal(newState, newAction);
			Q = Q +alpha*(reward + gamma * Qn-Q);
			qtable->setQVal(state, action, Q);
		...
		double get_reward(int state, int piece){
			...
			if(state == globe +numOfStates*piece) return 2.0;
			if(state == winRoad +numOfStates*piece) return 5.0;
			if(state == safety +numOfStates*piece) return 2.0;
			if(state == freeSpace +numOfStates*piece) return 1.0;
			if(state == danger +numOfStates*piece) return -1.0;
		}
